...
describe('okr-team', function() {
  ...
  const createAuthenticatedResponse = () => {
    return set(createOkrTeamResponse(), 'body.data.authenticationChecker.company.id', 'ID');
  };
  const createNotAuthenticatedResponse = () => {
    return set(createOkrTeamResponse(), 'body.data.authenticationChecker.company', null);
  };

  describe('parser', function() {
    it('should parse response and return 1 okr', function() {
      const okr = okrTeam.parser(createAuthenticatedResponse());
    ...
    it('should throw invalid token', function() {
      const fn = () => okrTeam.parser(createNotAuthenticatedResponse());
      expect(fn).to.throw('the token is invalid');
    });
  });
});
